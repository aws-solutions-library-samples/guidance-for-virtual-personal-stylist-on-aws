...
- For security purposes, you will also create lambda authorizer with api gateway. Api gateway uses the cognito user to parse the bearer token from the frontend application such that only authenticated users are able to invoke the api gateway - You can use this short video for using Amazon Verified Permissions to create this functionality in the app - [Lambda Authorizer setup](https://youtu.be/R7QuHGbKt5U)
